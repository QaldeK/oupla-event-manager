...
<div class="container mx-auto px-4 py-8">
  <div class="flex justify-between items-center mb-8">
    ...
    <div class="card bg-base-100 shadow-md p-4 max-w-md mx-auto">
      <div class="card-body p-4">
        <h2 class="card-title text-xl mb-2">Créer un nouveau pad</h2>
  ...
  {:else if pads.length === 0}
    <div class="card bg-base-100 shadow-md">
      <div class="card-body text-center py-16">
        <h3 class="text-xl font-semibold mb-2">Aucun document collaboratif</h3>
        <p class="text-base-content/60">
          Créez votre premier pad pour commencer à collaborer !
        </p>
      </div>
    </div>
  {:else}
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {#each pads as pad}
        <a href={`/dashboard/pads/${pad.id}`} class="card bg-base-100 shadow-md hover:shadow-lg transition-shadow">
          <div class="card-body">
            <h2 class="card-title text-xl">{pad.title}</h2>
            <p class="text-sm text-base-content/70">
              Créé le {formatDate(pad.created)}
            </p>
            <p class="text-sm text-base-content/70">
              Dernière modification: {formatDate(pad.updated)}
            </p>
