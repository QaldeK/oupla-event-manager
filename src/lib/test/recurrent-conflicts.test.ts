...
describe('Événements récurrents avec conflits', () => {
	...
	it('should handle errors gracefully during conflict update', async () => {
		...
		await expect(handleEventConflictsAfterSave(
			'NEW_RECURRENT',
			mockEventData as EventType,
			conflictIds,
			createdEventIds
	...
	it('should handle recurrent events with different conflict patterns', async () => {
		...
		const getConflictIds = (eventId: string) => {
			switch (eventId) {
				case 'event-1': return ['conflict-A', 'conflict-B'];
				case 'event-2': return ['conflict-C'];
				case 'event-3': return []; // Pas de conflit
				default: return [];
			}
		};
		...
		expect(mockUpdateEvent).toHaveBeenNthCalledWith(1, 'event-1', { inConflictWith: ['conflict-A', 'conflict-B'] });
		expect(mockUpdateEvent).toHaveBeenNthCalledWith(2, 'event-2', { inConflictWith: ['conflict-C'] });
