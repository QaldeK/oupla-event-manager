...
<form class="space-y-4" onsubmit={handleSubmit}>
	...
	<div>
		...
		{#if email && !email.includes("@")}
			<p class="text-error mt-1 text-xs">Veuillez saisir une adresse email valide.</p>
		{/if}
	</div>

	<div class="flex justify-end space-x-3 pt-4">
		<button type="button" class="btn" onclick={handleCancel} disabled={isLoading}> Annuler </button>
