<script lang="ts">
	...
	interface Props {
		displayedEvents: Array<EventType>;
		allFilteredEvents: Array<EventType>;
	...
	let showSideEventSum = $derived(screenDetector.isDesktop);
	...
	function formatEventDate(event: EventType): string {
		...
		if (event.dates_proposed && event.dates_proposed?.length > 0) {
										...
										<div class="flex items-center justify-between">
											<div
												class="text-base-content/60 rounded-lg px-2 text-xs {event.isConfirmed &&
													!event.canceled &&
													'bg-success/10'} {!event.isConfirmed &&
													!event.canceled &&
													'bg-warning/10'}
...
{:else}
	...
	<div class="fixed right-4 bottom-15 z-10">
		{#if isSearchOpen}
			<div
